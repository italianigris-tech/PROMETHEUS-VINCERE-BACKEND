 ...
 */
export const CAMERA_MOVE_CATALOG = [
...
export const CAMERA_MOVE_DEFINITION = Object.fromEntries(CAMERA_MOVE_CATALOG.map((def) => [def.kind, def]));
export const CAMERA_KINDS = CAMERA_MOVE_CATALOG.map((def) => def.kind);
// ---------------------------------------------------------------------------
// Math utilities
// ---------------------------------------------------------------------------
...
export const BASE_CAMERA = { position: [0, 0, 5], rotation: [0, 0, 0], fov: 45 };
...
const unitNoise = (seed, salt) => (hashSeed(seed, salt) / 0xffffffff) * 2 - 1;
// ---------------------------------------------------------------------------
// Per-frame evaluation
// ---------------------------------------------------------------------------
export function cameraProgress(plan, fps, frame) {
 ...
 */
export function evaluateCameraPose(plan, fps, frame, seed) {
    const progress = cameraProgress(plan, fps, frame);
    const rawProgress = clamp01((frame - plan.startSec * fps) / Math.max(1, (plan.endSec - plan.startSec) * fps));
    const pose = {
        position: [0, 0, 5],
        rotation: [0, 0, 0],
        fov: 45,
 ...
 */
export function decideLandscapeCameraMoves(sections, editMoves, typographyMoveIds, opts = {}) {
    ...
    sections.forEach((sec, idx) => {
        ...
        const duration = Math.min(sec.durationSec, Math.max(1.2, spec.defaultDurationSec * (0.8 + 0.4 * aggression)));
        plans.push({
