...
const { mirrorProgress, mirrorPose, mirrorOffsets, mirrorSpecsFromCatalog } =
  PARALLAX_MATH_MIRROR;
...
function blendPixel(dst: Uint8Array, i: number, rgb: number[], a: number): void {
  if (a >= 1) {
    dst[i] = rgb[0];
    dst[i + 1] = rgb[1];
    dst[i + 2] = rgb[2];
...
function blitTile(dst: Uint8Array, tile: Uint8Array, tileW: number, ox: number, oy: number): void {
  const oxi = ox | 0;
  const oyi = oy | 0;
  ...
  for (let y = y0; y < y1; y++) {
    ...
    for (let x = x0; x < x1; x++) {
      ...
      if (a > 0) blendPixel(dst, rowDst + x * 3, [tile[si], tile[si + 1], tile[si + 2]], a);
    }
  }
}

/** Copy a region of a larger plate onto dst at (ox, oy). */
function blitPlate(dst: Uint8Array, plate: Uint8Array, pw: number, ph: number, ox: number, oy: number): void {
  const srcX = -ox | 0;
  const srcY = -oy | 0;
  if (srcX < 0 || srcY < 0 || srcX + W > pw || srcY + H > ph) return;
  for (let y = 0; y < H; y++) {
...
function fillRect(dst: Uint8Array, x0: number, y0: number, x1: number, y1: number, rgb: number[], a = 1): void {
  ...
  for (let y = ry0; y <= ry1; y++) {
    const row = y * W * 3;
    for (let x = rx0; x <= rx1; x++) blendPixel(dst, row + x * 3, rgb, a);
  }
}

function hLine(dst: Uint8Array, x0: number, x1: number, y: number, rgb: number[], a = 1): void {
  const rx0 = Math.max(0, x0 | 0);
  const rx1 = Math.min(W - 1, x1 | 0);
  const ry = y | 0;
  if (ry < 0 || ry >= H) return;
...
function vLine(dst: Uint8Array, x: number, y0: number, y1: number, rgb: number[], a = 1): void {
  const rx = x | 0;
  const ry0 = Math.max(0, y0 | 0);
  const ry1 = Math.min(H - 1, y1 | 0);
  if (rx < 0 || rx >= W) return;
  for (let y = ry0; y <= ry1; y++) blendPixel(dst, y * W * 3 + rx * 3, rgb, a);
}

/** Axis-aligned rectangle rotated about its centre. */
function fillRectRot(
  dst: Uint8Array,
  cx: number,
  cy: number,
  w: number,
...
function drawText(dst: Uint8Array, str: string, x: number, y: number, scale: number, rgb: number[], a = 1): void {
  let cx = x;
...
function drawChip(dst: Uint8Array, x: number, y: number, text: string, color: number[]): void {
...
function buildBackgroundPlate(): Uint8Array {
...
function buildOrbTile(color: number[], core: number[]): Uint8Array {
...
function buildVignette(): Uint8Array {
...
function ampOff(off: { dx: number; dy: number; scale: number; rotationDeg: number }): PlaneOff {
  return {
    dx: off.dx * W * AMP_V,
    dy: off.dy * H * AMP_V,
...
function place(x: number, y: number, off: PlaneOff): [number, number] {
  const cx = W / 2;
  const cy = H / 2;
...
function line(dst: Uint8Array, x0: number, y0: number, x1: number, y1: number, rgb: number[], a: number): void {
  const dx = x1 - x0;
  const dy = y1 - y0;
  ...
  for (let t = 0; t <= len; t++) {
    ...
    if (x >= 0 && x < W && y >= 0 && y < H) blendPixel(dst, y * W * 3 + x * 3, rgb, a);
  }
}

function familyColor(f: string): number[] {
  switch (f) {
    case "lens":
      return C.amber;
...
// ---------------------------------------------------------------------------
interface Segment {
  def: (typeof CAMERA_MOVE_CATALOG)[number];
  startSec: number;
  endSec: number;
  durationSec: number;
  plan: { kind: string; startSec: number; endSec: number; intensity: number };
  index: number;
  total: number;
}

function buildSegments(kinds?: string[]): Segment[] {
  let t = INTRO_SEC;
...
const specs = mirrorSpecsFromCatalog(
  CAMERA_MOVE_CATALOG as unknown as Array<{ kind: string; defaultEasing: string; defaultDurationSec: number }>,
);

function findSegment(segs: Segment[], sec: number): Segment {
  if (sec < INTRO_SEC) return segs[0];
  for (const s of segs) if (sec < s.endSec) return s;
  return segs[segs.length - 1];
}

const fmt = (v: number, d: number): string => v.toFixed(d);


// ---------------------------------------------------------------------------
// Per-frame scene render
// ---------------------------------------------------------------------------
function drawScene(dst: Uint8Array, offBG: PlaneOff, offMID: PlaneOff, offFG: PlaneOff): void {
  // background plate
...
function drawTracker(dst: Uint8Array, offBG: PlaneOff, offMID: PlaneOff, offFG: PlaneOff): void {
  const rows: Array<[string, number[], PlaneOff]> = [
    ["BG", C.cyan, offBG],
    ["MID", C.violet, offMID],
    ["FG", C.amber, offFG],
  ];
...
function drawHud(
  ...
  seg: Segment,
  p: number,
  pose: { position: number[]; rotation: number[]; fov: number },
  offBG: PlaneOff,
  offMID: PlaneOff,
  offFG: PlaneOff,
): void {
  ...
  drawText(
    dst,
    `FAMILY ${seg.def.family} · EASING ${seg.def.defaultEasing} · ${seg.durationSec.toFixed(1)}S · AMP x${AMP_V.toFixed(1)}`,
    24,
    24,
    1,
    C.dim,
...
function drawProgressBar(dst: Uint8Array, cx: number, y: number, w: number, p: number, color: number[]): void {
...
// Intro title card
// ---------------------------------------------------------------------------
function renderIntroCard(dst: Uint8Array, frame: number): void {
...
function applyVignette(dst: Uint8Array): void {
  for (let i = 0; i < W * H; i++) {
...
function addGrain(dst: Uint8Array, frame: number): void {
  for (let y = 0; y < H; y += 2) {
    const row = y * W * 3;
    for (let x = 0; x < W; x += 2) {
...
// Frame driver
// ---------------------------------------------------------------------------
function renderFrame(frame: number, segs: Segment[], totalFrames: number): Uint8Array {
...
async function main(): Promise<void> {
  ...
  if (args.fps) FPS_V = Math.max(12, Math.min(60, parseInt(args.fps, 10)));
  if (args.amp) AMP_V = Math.max(1, Math.min(6, parseFloat(args.amp)));
  ...
  console.error(
    `[camera-proof] ${segs.length} moves → ${totalSec.toFixed(1)}s @ ${FPS_V}fps = ${totalFrames} frames, AMP x${AMP_V}`,
  );
  ...
  if (!ff.stdin) throw new Error("no stdin");
