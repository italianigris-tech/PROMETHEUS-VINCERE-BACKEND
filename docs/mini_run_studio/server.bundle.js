...
// docs/mini_run_studio/serve_preview.ts
var http = __toESM(require("node:http"));
var fs = __toESM(require("node:fs"));
var path = __toESM(require("node:path"));
var import_node_child_process = require("node:child_process");
var studioDir = __dirname;
var repoRoot = path.resolve(studioDir, "../..");
var sfxVideoPath = path.join(studioDir, "video_with_real_sfx.mp4");
var mutedVideoPath = path.join(studioDir, "uploaded_input_video.mp4");
var soundJsonPath = path.join(studioDir, "authoritative_sound_treatment.json");
var soundFxDir = path.join(repoRoot, "SOUND FX");
var renderScript = path.join(studioDir, "render_video_with_sfx.ts");
var MIME_TYPES = {
...
var ultraLightStudioHtml = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
    ...
    <div class="studio-grid">
      ...
      <div class="card">
        ...
        <div class="loop-banner" id="loopBanner">
          <div>
            \u{1F501} <strong>Micro-Loop Active:</strong> <span id="loopRangeText">00:00.00 - 00:00.00</span>
      ...
      <div class="card">
        ...
        <div class="tab-bar">
          <div class="tab-btn-group">
            <button class="tab-btn active" id="tabTableBtn" onclick="showTab('table')">\u{1F4CA} Sound Treatments (282 Cues)</button>
...
async function extractBinaryPayload(buffer, contentType) {
  ...
  if (contentType && contentType.includes("application/json")) {
    try {
      ...
      let filename = parsed.filename || parsed.name || void 0;
  ...
  if (contentType && contentType.includes("multipart/form-data")) {
    ...
    if (boundaryMatch) {
      ...
      if (headerEndIndex !== -1) {
        ...
        const parsedFilename = filenameMatch ? filenameMatch[1].trim() : void 0;
...
function renderDropzonePageHtml(studioDir2) {
  const uploadDir = path.join(studioDir2, "uploaded_screenshots");
  let items = [];
  if (fs.existsSync(uploadDir)) {
    const files = fs.readdirSync(uploadDir).filter((f) => f.endsWith(".png") || f.endsWith(".jpg") || f.endsWith(".jpeg") || f.endsWith(".webp") || f.endsWith(".gif") || f.endsWith(".svg"));
    ...
    initialCardsHtml = items.map((item) => `
      <div class="item-card">
        <div class="img-wrap" onclick="openLightbox('${item.url}', '${item.name}')">
          ...
          <div class="expand-badge">\u{1F50D} Expand</div>
        </div>
        ...
        <div class="item-btn-bar">
          <button class="btn-item" type="button" onclick="event.stopPropagation(); copyToClipboard(window.location.origin + '${item.url}', 'Copied image link!')">\u{1F4CB} Copy Link</button>
          <button class="btn-item" type="button" onclick="event.stopPropagation(); openLightbox('${item.url}', '${item.name}')">\u{1F441}\uFE0F View</button>
          <button class="btn-item btn-item-del" type="button" onclick="event.stopPropagation(); deleteSingleImage('${item.name}', this.closest('.item-card'))">\u{1F5D1}\uFE0F Delete</button>
        </div>
      </div>
    `).join("\n");
  }
...
<body>
  ...
  <div class="container">
    <div class="header">
      <h1>\u{1F4F8} Screenshot Dropzone & Reference Gallery</h1>
      <p>Instant paste (Ctrl+V / \u2318V), drag & drop, and automated screenshot ingestion hub</p>
      <div class="nav-links">
        <a href="/">\u{1F3B5} Sound Studio</a>
        <a href="/typo">\u{1F524} Typography Studio (/typo)</a>
        <a href="/paste" class="active">\u{1F4F8} Screenshots Gallery (/paste)</a>
      </div>
    </div>

    <div class="dropzone" id="dropzone" tabindex="0" role="button" aria-label="Dropzone for screenshot uploads">
      <div class="dropzone-icon">\u{1F4CB}</div>
    ...
    <div class="gallery-bar">
      ...
      <div class="gallery-actions">
        <input type="text" class="search-box" id="searchBox" placeholder="\u{1F50D} Search images..." />
        <button class="btn-tool" id="btnRefresh" type="button" title="Refresh Gallery">\u{1F504} Refresh</button>
        <button class="btn-tool btn-danger" id="btnDeleteAll" type="button" onclick="deleteAllImages()" title="Delete All Images">\u{1F5D1}\uFE0F Delete All</button>
      </div>
    </div>
    ...
    <div class="lightbox-content">
      ...
      <div class="lightbox-bar">
        ...
        <div style="display: flex; gap: 8px;">
          <button class="btn-tool" id="lightboxCopy" type="button">\u{1F4CB} Copy Link</button>
  ...
  <script>
    ...
    function renderGallery() {
      ...
      filtered.forEach(item => {
        ...
        copyBtn.type = 'button';
        copyBtn.innerHTML = '\u{1F4CB} Copy Link';
        copyBtn.onclick = (e) => {
          e.stopPropagation();
        ...
        viewBtn.type = 'button';
        viewBtn.innerHTML = '\u{1F441}\uFE0F View';
        viewBtn.onclick = (e) => {
          e.stopPropagation();
        ...
        delBtn.type = 'button';
        delBtn.innerHTML = '\u{1F5D1}\uFE0F Delete';
        delBtn.onclick = (e) => {
          e.stopPropagation();
      ...
      if (cd.files && cd.files.length > 0) {
        for (let i = 0; i < cd.files.length; i++) {
          if (cd.files[i].type && cd.files[i].type.startsWith('image/')) {
            imageBlob = cd.files[i];
            filename = cd.files[i].name;
            break;
          }
        }
      }
    ...
    document.getElementById('btnPasteClipboard').addEventListener('click', async (e) => {
      ...
      showToast('\u{1F4CB} Press Ctrl+V (or \u2318V on Mac) now to paste from your clipboard!', 'uploading', 4500);
    ...
    document.getElementById('btnRefresh').addEventListener('click', (e) => {
      e.stopPropagation();
      showToast('\u{1F504} Refreshing gallery...', 'info', 1200);
      loadGallery();
    });
...
function createServerInstance(port) {
  const s = http.createServer((req, res) => {
    ...
    if ((req.method === "GET" || req.method === "HEAD") && (req.url === "/" || req.url === "/studio" || req.url === "/video")) {
    ...
    if (req.method === "POST" && req.url === "/api/rebake_audio") {
      try {
        ...
        (0, import_node_child_process.execSync)(`npx tsx "${renderScript}"`, { stdio: "inherit" });
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ success: true }));
      } catch (e) {
        res.writeHead(500, { "Content-Type": "application/json" });
    ...
    try {
      decodedUrl = decodeURIComponent(rawPathname);
    } catch {
    }
    ...
    if ((req.method === "GET" || req.method === "HEAD") && (req.url?.startsWith("/typo") || req.url?.startsWith("/typography_treatment_presentation.html") || req.url === "/presentation")) {
    ...
    if ((req.method === "GET" || req.method === "HEAD") && (req.url === "/paste" || req.url === "/dropzone" || req.url === "/screenshots" || req.url?.startsWith("/paste?"))) {
      ...
      req.on("end", async () => {
        try {
          ...
          try {
            ...
            parsedFilename = urlObj.searchParams.get("filename") || void 0;
          } catch {
          }
          ...
          console.log(`\u{1F4F8} [UPLOAD_SUCCESS] Saved screenshot: ${finalFilename} (${data.length} bytes)`);
          res.writeHead(200, { "Content-Type": "application/json" });
      ...
      req.on("end", () => {
        try {
          ...
          if (fs.existsSync(filePath)) {
            ...
            console.log(`\u{1F5D1}\uFE0F [DELETE_SUCCESS] Deleted single screenshot: ${safeFilename}`);
          }
          res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if (req.method === "POST" && (req.url === "/api/delete_all_screenshots" || req.url?.startsWith("/api/delete_all_screenshots"))) {
      try {
        ...
        console.log(`\u{1F5D1}\uFE0F [DELETE_ALL_SUCCESS] Deleted ${count} screenshots from uploaded_screenshots/`);
        res.writeHead(200, { "Content-Type": "application/json" });
  ...
  s.listen(port, "0.0.0.0", () => {
    console.log(`  \u{1F680} Interactive Micro-Looping Studio on Port ${port}: http://16.192.95.115:${port}/`);
  });
  s.on("error", (e) => {
...
var server = createServerInstance(8080);
process.on("SIGINT", () => {
  server.close();
  process.exit(0);
});
