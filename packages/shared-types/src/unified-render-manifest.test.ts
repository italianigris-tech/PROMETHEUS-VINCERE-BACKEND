...
describe("UnifiedRenderManifestSchema", () => {
  ...
  it("accepts an inspectable Joseph semantic macro-rig contract for proof/data/exhibit moments", () => {
    const pipPlan = {
      version: "joseph-pip-v1",
      layout: "speaker_right_text_left",
      sourceTrackId: "primary",
    ...
    expect(manifest.josephMacroRig?.rigId).toBe("talking-head-proof-data-exhibit");
    expect(manifest.josephMacroRig?.inputs.requiredSceneRoles).toEqual(
      expect.arrayContaining(["talking_head", "proof", "data_exhibit"]),
    );
    expect(manifest.josephMacroRig?.renderFields.typographySlots.map((slot) => slot.role)).toEqual(
      expect.arrayContaining(["proof_headline", "data_label"]),
    );
    expect(manifest.josephMacroRig?.failureFallbacks.map((fallback) => fallback.action)).toEqual(
