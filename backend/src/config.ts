...
const envSchema = z.object({
  ...
  JOB_QUEUE_DRIVER: z.enum(["in_process", "bullmq"]).default("in_process"),
  REDIS_URL: z.string().default(""),
  JOB_QUEUE_PREFIX: z.string().trim().min(1).default("prometheus"),
  JOB_QUEUE_ATTEMPTS: z.coerce.number().int().positive().default(3),
  JOB_QUEUE_BACKOFF_MS: z.coerce.number().int().nonnegative().default(1000),
  JOB_QUEUE_TIMEOUT_MS: z.coerce.number().int().positive().default(15 * 60 * 1000),
  JOB_QUEUE_SHUTDOWN_GRACE_MS: z.coerce.number().int().positive().default(30000),
