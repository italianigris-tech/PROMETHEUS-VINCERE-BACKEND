...
export const selectMaulTypographySfx = ({
  ...
  const selected: Array<MaulTypographySfxAsset & {sourceMs: number}> = [];
  ...
  for (const program of [...programs].sort(
    ...
    const eventType = emphasis.test(program.treatment)
      ? "typography_emphasis"
    ...
    selected.push({...asset, id: `${asset.id}_${selected.length + 1}`, sourceMs});
...
export const materializeAutomaticMaulSfx = (
  programs: readonly MaulTypographyAnimationProgram[],
) => selectMaulTypographySfx({programs}).map((asset) => ({
  ...asset,
