...
describe("MaulWorkerExecutor", () => {
  ...
  it("completes one leased render job with governed artifact identifiers", async () => {
    ...
    const projects = {
      ...
      reviewCandidate: async () => ({review: {artifactId: "review_1"}}),
      renderShort: async () => ({export: {artifactId: "export_1"}}),
    };
    ...
    await expect(executor.runOnce()).resolves.toMatchObject({status: "completed", result: {exportArtifactId: "export_1", qualityTruthStatus: "passed"}});
  });
